 ...
 *
 * Known limitation: Does not catch references as Anonymous Inner class: def x = new java.sql.Statement() { .. }
 *
 * @author Chris Mair
 */
class JdbcStatementReferenceRule extends AbstractAstVisitorRule {
    String name = 'JdbcStatementReference'
    int priority = 2

    @Override
    AstVisitor getAstVisitor() {
        new ClassReferenceAstVisitor('java.sql.Statement, java.sql.PreparedStatement, java.sql.CallableStatement')
    }
}
