...
class GAstUtil {
    ...
    static boolean isPropertyNamed(Expression property, expectedName) {
        return (property instanceof PropertyExpression && GAstUtil.isConstant(property.property, expectedName))
    }
    ...
    static boolean isFinalVariable(DeclarationExpression declarationExpression, SourceCode sourceCode) {
        ...
        def variableExpressions = GAstUtil.getVariableExpressions(declarationExpression)
    ...
    static boolean isMethodNode(ASTNode node, String methodNamePattern, Integer numArguments = null, Class returnType = null) {
        ...
        if (returnType && !GAstUtil.classNodeImplementsType(node.returnType, returnType)) {
            return false
        }
        true
    }
    ...
    static boolean isNotNullCheck(expression) {
        if (expression instanceof BinaryExpression && expression.operation.text == '!=') {
            if (GAstUtil.isNull(expression.leftExpression) || GAstUtil.isNull(expression.rightExpression)) {
                return true
            }
        }
        false
    ...
    static String getNullComparisonTarget(expression) {
        if (expression instanceof BinaryExpression && expression.operation.text == '!=') {
            if (GAstUtil.isNull(expression.leftExpression)) {
                return expression.rightExpression.text
            } else if (GAstUtil.isNull(expression.rightExpression)) {
    ...
    private static Class getClassForClassNode(ClassNode type) {
        ...
        } else if (GAstUtil.classNodeImplementsType(type, String)) {
            return String
        } else if (GAstUtil.classNodeImplementsType(type, ReentrantLock)) {
    ...
    private static Class getPrimitiveType(ClassNode type) {
        if (GAstUtil.classNodeImplementsType(type, Boolean) || GAstUtil.classNodeImplementsType(type, Boolean.TYPE)) {
            return Boolean
        } else if (GAstUtil.classNodeImplementsType(type, Long) || GAstUtil.classNodeImplementsType(type, Long.TYPE)) {
            return Long
        } else if (GAstUtil.classNodeImplementsType(type, Short) || GAstUtil.classNodeImplementsType(type, Short.TYPE)) {
            return Short
        } else if (GAstUtil.classNodeImplementsType(type, Double) || GAstUtil.classNodeImplementsType(type, Double.TYPE)) {
            return Double
        } else if (GAstUtil.classNodeImplementsType(type, Float) || GAstUtil.classNodeImplementsType(type, Float.TYPE)) {
            return Float
        } else if (GAstUtil.classNodeImplementsType(type, Character) || GAstUtil.classNodeImplementsType(type, Character.TYPE)) {
            return Character
        } else if (GAstUtil.classNodeImplementsType(type, Integer) || GAstUtil.classNodeImplementsType(type, Integer.TYPE)) {
            return Integer
        } else if (GAstUtil.classNodeImplementsType(type, Long) || GAstUtil.classNodeImplementsType(type, Long.TYPE)) {
            return Long
        } else if (GAstUtil.classNodeImplementsType(type, Byte) || GAstUtil.classNodeImplementsType(type, Byte.TYPE)) {
            return Byte
        }
        null
    }
    ...
    static int findFirstNonAnnotationLine(ASTNode node, SourceCode sourceCode) {
        if (node instanceof AnnotatedNode && node.annotations) {
            ...
            def rawLine = GAstUtil.getRawLine(sourceCode, lastAnnotation.lastLineNumber-1)
    ...
    static boolean expressionIsNullCheck(ASTNode node) {
        ...
        if (GAstUtil.isBinaryExpressionType(booleanExp.expression, '==')) {
            if (GAstUtil.isNull(booleanExp.expression.leftExpression) && booleanExp.expression.rightExpression instanceof VariableExpression) {
                return true
            } else if (GAstUtil.isNull(booleanExp.expression.rightExpression) && booleanExp.expression.leftExpression instanceof VariableExpression) {
                return true
            }
    ...
    static boolean expressionIsAssignment(ASTNode node, String variableName) {
        if (node instanceof Expression && GAstUtil.isBinaryExpressionType(node, '=')) {
            if (GAstUtil.isVariable(node.leftExpression, variableName)) {
                return true
            }
        } else if (node instanceof ExpressionStatement && GAstUtil.isBinaryExpressionType(node.expression, '=')) {
            if (GAstUtil.isVariable(node.expression.leftExpression, variableName)) {
                return true
            }
        }
        false
    ...
    static String getDeclaration(ASTNode node, SourceCode sourceCode) {
        ...
        for (lineIndex in (node.lineNumber-1 .. node.lastLineNumber-1)) {
            ...
            def line = GAstUtil.getRawLine(sourceCode, lineIndex)
